...
if (context != null)
{
  ...
  if (preprocessing)
  {
    context["ToolTip"] = "n=<double> (index of refraction)\rrc[=<bool>] (ray-casting mode)";
    return;
  }

  // Optional IImageFunction.
  context["Algorithm"] = rc ? new RayCasting() : (RayCasting)new RayTracing();
}
...
// Glass.
PhongMaterial glass = rc
  ? new PhongMaterial(new double[] {0.1, 0.3, 1.0}, 0.1, 0.8, 0.1, 32)
  : new PhongMaterial(new double[] {0.0, 0.2, 0.1}, 0.05, 0.05, 0.1, 128);
glass.n  = n;
glass.Kt = 0.9;
